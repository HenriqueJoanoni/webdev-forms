...
function nutritionInformation(arrayData, ev) {
    ...
    if (info.hasOwnProperty("nutrition-per-100g")) {
        modalHeader = `<h3>Nutritional Information per 100 g</h3><ul>`;
        modalContentHTML = generateNutritionContent(info["nutrition-per-100g"], modalHeader);
    }

    if (info.hasOwnProperty("nutrition-per-100ml")) {
        modalHeader = `<h3>Nutritional Information per 100 ml</h3><ul>`;
        modalContentHTML = generateNutritionContent(info["nutrition-per-100ml"], modalHeader);
    }

    modal.querySelector(".modal-content").innerHTML = modalContentHTML;
...
function generateNutritionContent(nutritionData, header) {
    let html = header;
    for (const key in nutritionData) {
        html += `<li>${key}: ${nutritionData[key]}</li>`;
    }
    html += `</ul>`;
    return html;
}
